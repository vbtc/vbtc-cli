...
class BlinkTradeCli(object):
    ...
    def send_message(self, msg):
        ...
        nonce = str(
            int((time.mktime(dt.timetuple()) + dt.microsecond / 1000000.0) * 1000000)
        )
        signature = hmac.new(
            str(self._secret).encode(), str(nonce).encode(), digestmod=hashlib.sha256
        ).hexdigest()
        ...
        try:
            response = requests.post(
                url, json=msg, verify=True, headers=headers, timeout=TIMEOUT_IN_SECONDS
            )
    ...
    def get_list(self, list_msg_type, list_name, params, page=0, num_pages=1):
        ...
        while still_have_records_to_read:
            ...
            msg = {"MsgType": list_msg_type, "Page": page, "PageSize": 20}
    ...
    def list_withdrawals(self, status=None, page=0, num_pages=1):
        ...
        res = self._handle_withdraw_list(
            self.get_list(
                "U26", "WithdrawList", {"StatusList": status_list}, page, num_pages
            )
        )
    ...
    def list_deposits(self, status=None, page=0, num_pages=1):
        ...
        res = self._handle_deposit_list(
            self.get_list(
                "U30", "DepositList", {"StatusList": status_list}, page, num_pages
            )
        )
        self._printout_result(res[0], res[1])
    ...
    def list_users(self, status=1, page=0, num_pages=1, filter=None):
        ...
        res = self._handle_user_list(
            self.get_list(
                "B2",
                "CustomerList",
                {"StatusList": status_list, "Filter": filter},
                page,
                num_pages,
            )
        )
            ...
            print(
                json.dumps(
                    [
                        ...
                        api_response["Detail"] if "Detail" in api_response else "",
                    ]
                )[1:-1]
            )
            return
    ...
    def _printout_deposit_address_response(self, response):
        if self._show_header:
            print(
                '"DepositID", "InputAddress", "State", "CreditProvided", "Value", "PaidValue"'
            )
        print(
            json.dumps(
                [
    ...
    def _extract_user_verification_data(
        self, rec, verification_data_field="VerificationData"
    ):
        name = None
        phone_number = None
        identification = None
            ...
            for d in rec[verification_data_field]:
                if "verification" in d:
                    if "service_provider" in d["verification"]:
                        if (
                            "blinktrade_device_id_checker"
                            == d["verification"]["service_provider"]
                            and "yes" == d["verification"]["match_other_accounts"]
                        ):
                ...
                if "name" in d:
                    name = " ".join(
                        [d["name"]["first"], d["name"]["middle"], d["name"]["last"]]
                    )
                ...
                if "date_of_birth" in d:
                    ...
                    if (
                        len(date_of_birth_split) == 3
                        and len(date_of_birth_split[1]) < 3
                    ):
                        date_of_birth = datetime.date(
                            year=int(date_of_birth_split[0]),
    ...
    def _handle_deposit_list(self, deposit_list):
        ...
        for rec in deposit_list:
            ...
            name = ""
            if (
                "UserVerificationData" in rec
                and rec["UserVerificationData"] is not None
            ):
